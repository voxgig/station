/* The pure half of profile handling: selection, the ref grammar, and
 * the §3.3 merge. Split from `profile.ts` so the BROWSER ENTRY can
 * reach it (§2.2, §17 Phase 1): everything here is data-in/data-out,
 * while the file half (findConfigFile/loadConfig/configScope) is
 * node-only and stays in `profile.ts`, imported by the node entry
 * alone. `profile.ts` re-exports these, so every existing import path
 * is unchanged. */
...
// open() opts winning).
//
// `process` is reached through a typeof guard rather than an import:
// in the browser there is no process and no environment, so the answer
// is simply the option or the default - which is the same degradation
// "no env var set" already means.
export function selectProfile(optProfile?: string): string {
  if (null != optProfile && '' !== optProfile) { return optProfile }
  const env = 'undefined' !== typeof process
    ? process.env.VOXGIG_STATION_PROFILE : undefined
