/* Profile handling, whole: the pure half - selection, the ref grammar
 * and the §3.3 merge - and the file half that finds and reads
 * station.json plus the scope rule hanging off where it was found.
 *
 * These were two modules. `profilecore.ts` held the pure half so the
 * BROWSER ENTRY could reach it without pulling `node:fs`, and `Station`
 * reached the file half through a `ConfigFileIO` seam the node entry
 * registered. Station is server-side only, so the split, the seam and
 * the second entry are all gone and this is one module again. Every
 * import of `./profile` is unchanged - that path always re-exported the
 * pure half. */
...
// open() opts winning).
//
// `process` is read directly. It used to go through a `typeof` guard
// because the browser entry reached this code and has no `process`;
// station is server-side only, so the guard guarded nothing.
