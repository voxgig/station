...
const { structmod } = require('./structhome')
const { StationError } = require('./error')
const { envtoken } = require('./descriptor')

const { clone, validate } = structmod()
...
// reads. This port runs straight from src in the repo (like omnihome's
// specfile), so it reads the JSON itself rather than shipping a mirror
// the way the published TypeScript package must. Every validate gets a
// CLONE: struct's validate consumes the spec it walks (it deletes
// satisfied `$ONE` branches as it goes), so handing it the parsed
// constant twice would validate the second config against a spec the
// first had already eaten.
let CONFIG_SHAPE = null

function configShape() {
  if (null == CONFIG_SHAPE) {
    const file = Path.resolve(
      Path.join(__dirname, '..', '..', 'spec', 'config-shape.json'))
    CONFIG_SHAPE = JSON.parse(Fs.readFileSync(file, 'utf8'))
  }
...
// errors, which is this port's one structural deviation from the
// canonical two-throw order: a struct new enough to reject a
// first-element gap itself reports a DIFFERENT spelling ("to be one
// of ..."), and the corpus pins the explicit one - so the pinned
// message is produced here either way, and behavior is identical
// whatever struct version resolves.
//
...
function validateConfig(normalized) {
  ...
  const { secrets, reserved, invalid } = scanConfig(normalized)

  if (0 < errs.length || 0 < invalid.length) {
    throw new StationError('station_config_invalid',
      errs.concat(invalid).join('; ') + renamehint(normalized))
  }
...
// Collects rather than throws - validateConfig owns the throw order.
function scanConfig(cfg) {
  const secrets = []
  const reserved = []
  const invalid = []
...
function checkfeatures(f, path, secrets, reserved, invalid) {
  if (!ismap(f)) { return }
  for (const name of Object.keys(f)) {
...
function checkpolicy(policy, path, invalid) {
...
function firstelement(list, path, invalid) {
...
function scan(node, path, secrets, reserved) {
...
function secretvalue(val, path, secrets) {
...
function userinfo(val, path, secrets) {
