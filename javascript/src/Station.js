...
function instanceRef(api, fopts) {
...
class Station {
  ...
  // §6.2's second path, and the front door the docs name. Delegates to
  // the same process-global table the free function fills; there is one
  // registry, not two.
  static provide(api, factory) {
    provide(api, factory)
  }
  ...
  options(a, b) {
    ...
    const instance = named ? a : undefined
  ...
  // at 26 instances over 20 apis that is 20 normalizations, not 26, and
  ...
  // only API-stable metadata - which is what factory.js already does at
  // provide time. Per-instance activation is `featuresOf(name)`'s
  // answer, and always was; a cache keyed by slug but built from the
  // first instance's feature map would make descriptorOf()
  // construction-order-dependent.
  describe(config, _feature) {
  ...
  async _transport(name, inner, fctx, fullurl, fetchdef) {
    ...
    if (live && null != entry && 'R1' === entry.rung) {
      ...
      //
      // No fallback: this branch is guarded by `'R1' === entry.rung`,
      // which is set only when `descriptor.auth.active` - the same
      // condition under which `entry.secretname` is populated.
      // Substituting a different name when it is absent is the bug
      // above, written again.
  ...
  emitErr(name, fctx, err) {
    this.emit({
      ...
      // by api and then get no api for every request, error and hoist
      // from the same client - the grouping working exactly until it
      // was used.
  ...
  build(name, as, overrides) {
    ...
    // alone left two gaps: production `sdk()` silently ignored an
    // unknown option like `retry.retires`, and `check()` itself missed
    // the case where the factory is discovered by the LOADER (its
    // pre-check sees no registered factory, then `sdk()` loads and
    // constructs unvalidated). One call here closes both, because every
    // path to a constructor comes through this line.
    const faults = checkfeatures(this.featuresOf(name).merged, entry.descriptor)
    if (0 < faults.length) {
      throw new StationError(faults[0].code,
    ...
    //
    // THE ALIAS IS RECORDED, NOT THE FIELDS. Carrying the declared
    // `secret` through the feature options and stopping there left
    // `policy`, `base` and everything else behind, so an auto-tagged
    // client silently lost its declared instance's HOSTS ALLOWLIST and
    // fell back to the wider api-level one. Carrying configuration a
    // field at a time is how that happens; recording what the tag
  ...
  features(filter) {
    ...
    const want = filter?.feature
    if (null == want) { return rows }
    return rows
      .filter((row) => null != row.merged[want])
      .map((row) => ({
        instance: row.instance,
        api: row.api,
        ordered: row.ordered.filter((n) => n === want),
  ...
  async warm(names) {
    ...
    // CONCURRENT, not serial. Awaiting inside the loop makes `warm`
    ...
    for (const name of wanted) {
      ...
      // included (§9). A NAME NOBODY DECLARED OR REGISTERED IS A MISS,
      // not a lookup - a wider fallback would let a typo like
    ...
    const results = await Promise.all(
      Array.from(bysecret.entries()).map(async ([secretname, snames]) => {
        try {
          await this.broker.value(snames[0], secretname)
          return { names: snames, ok: true }
        }
        catch (_e) { return { names: snames, ok: false } }
      }))
