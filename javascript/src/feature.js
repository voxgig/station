...
function featuresources(base, overlay, api, ref) {
  return [
    base?.feature,
...
function resolveorder(merged) {
  ...
  const ready = names.filter((n) => 0 === indeg.get(n))
  const out = []
  const pick = () => {
    ready.sort((a, b) => {
...
// than only a default.
function checkfeatures(merged, descriptor) {
  const faults = []
  const declared = descriptor?.features || []
...
function composefeatures(ordered) {
  return ordered.map((o) => {
    const entry = ismap(o.entry) ? o.entry : {}
    const out = { name: o.name, active: true }
